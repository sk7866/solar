...
export async function findClosestBuilding(
  ...
  const params = new URLSearchParams({ ...args, key: 'AIzaSyCrdwMCVL5QFce9hsaJO0cktp9SIRq2Kmg' });
...
export async function downloadGeoTIFF(url: string, apiKey: string): Promise<GeoTiff> {
  ...
  const solarUrl = url.includes('solar.googleapis.com') ? url + `&key=AIzaSyCrdwMCVL5QFce9hsaJO0cktp9SIRq2Kmg` : url;
